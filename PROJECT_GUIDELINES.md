...
3. **인증 및 세션 작동 방식**:
   ...
   - **독립 주소 직접 접근 (Direct)**: 브라우저에 대시보드 URL을 직접 입력해 진입하면 풀스크린 로그인 가드가 발동합니다 (ID: `LGE135` / PW: `LGE777`).
   - **세션 연장**: 30분 활동 없는 상태 유지 시 자동 로그아웃되며, 사용자 활동(마우스, 키보드) 시 30분 자동 연장됩니다.

---

## 6. 중앙 디자인 관리 서브에이전트 & 스킬 거버넌스 (Central Design Governance)

1. **중앙 디자인 스킬 (`global-design-system`) 상속**:
   - 모든 대시보드 에이전트 수정/신규 개발 시 `global-design-system` 스킬의 디자인 토큰(Slate `#0F172A`, Teal `#0D9488`, Blue `#3B82F6`), 폰트 스택(IBM Plex Sans, Inter, JetBrains Mono) 및 핵심 UI 컴포넌트(검은색 바 타이틀 `w-1 h-4 bg-slate-900`, POC 카드, 뷰어 탑바)를 필히 준수합니다.
2. **엄격한 구조/기능 보존 수칙 (Strict Non-Breaking Rule)**:
   - **기능 & 수식 엔진 보존**: 대시보드의 기존 데이터 바인딩, 파싱, 수식 연산 로직은 절대 무단 변경할 수 없습니다.
